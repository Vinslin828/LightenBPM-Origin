...
import { useImportCheck, useImportExecute } from "@/hooks/useForm";
import {
  ExportPayload,
  ImportCheckResponse,
...
export default function ImportFormModal({ onImportSuccess, ...props }: Props) {
  const { t } = useTranslation();
  const { toast } = useToast();
  const { mutateAsync: importCheck } = useImportCheck();
  const { mutateAsync: importExecute } = useImportExecute();
      ...
      <div className="p-7.5 flex flex-col gap-7.5 items-center min-w-[400px]">
        ...
        {step === "checking" && (
          <div className="w-full flex flex-col gap-4 items-center py-8">
            <div className="w-8 h-8 border-3 border-lighten-blue border-t-transparent rounded-full animate-spin" />
            <p className="text-sm text-secondary-text">
              {t("import.checking")}
            </p>
          </div>
        ...
        {(step === "review" || step === "executing") && checkResult && (
          <>
            <div className="relative w-full flex flex-col gap-4">
              {/* Loading overlay during executing */}
              {step === "executing" && (
            ...
            {/* Only show action buttons during review, not executing */}
            {step === "review" && (
            ...
            <span
              className={`ml-auto text-xs font-medium ${
                item.severity === "BLOCKING"
                  ? "text-red-500"
                  : item.status === "MISSING" || item.severity === "WARNING"
                    ? "text-amber-500"
                    : "text-green-500"
              }`}
            >
...
function StatusIcon({
  ...
  if (status === "MISSING" || severity === "WARNING") {
