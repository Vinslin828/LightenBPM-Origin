...
# Runs Prisma migration as an ECS Fargate task
#
...
if [ -z "$STAGE" ]; then
  ...
  echo "Example: STAGE=dev ./run-migration.sh"
  exit 1
fi
...
# Run migration as ECS Fargate task
...
echo ""
echo "Migration task stopped. Checking result..."
