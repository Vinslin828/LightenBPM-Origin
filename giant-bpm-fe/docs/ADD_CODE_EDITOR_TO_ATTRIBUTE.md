# How to Add Reference Support with CodeEditButton

This guide explains how to add "reference mode" to an attribute, using
`<CodeEditButton />` instead of a plain `<input />` for editing expressions.
...
In the attribute `definition.ts`, change validation to accept the
`{ isReference, reference, value }` shape.
...
- Use `useCode().validateReference` for reference validation.
- Render manual input normally.
- For reference mode, use `<CodeEditButton />` to open a code editor modal
...
{attributeValue.isReference ? (
  <>
    <CodeEditButton onClick={() => setIsEditorOpen(true)} />
    <CodeEditorModal
      isOpen={isEditorOpen}
      ...
      onSave={(nextValue) => {
        ...
        props.setValue({ ...attributeValue, reference: nextValue });
      }}
    />
  </>
) : (
...
Notes:
- Use the modal for editing and validation, so the attribute panel stays clean.
- Keep the latest editor value in `attributeValue.value`.
...
If you are not already using it, use:
- `CodeEditorModal` (modal wrapper + validation)
- `CodeTextarea` (textarea with line numbers)
...
- Attribute component renders CodeToggle + CodeEditButton + modal
