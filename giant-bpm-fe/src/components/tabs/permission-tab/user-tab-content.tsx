  ...
  return (
    <>
      <Input
        ...
        name="search-user-name-email"
        data-test-id="permission-user-search"
        value={searchQuery}
...
export function UserTile({ user }: UserTileProps) {
  return (
    <div className="border-stroke inline-flex justify-start items-center gap-4 min-w-0 max-w-full">
      <div className="flex-1 flex justify-start items-center gap-2.5 min-w-0">
        <Avatar name={user.name} size="sm" colorScheme="blue" />
        <div className="flex-1 inline-flex flex-col justify-end items-start min-w-0">
          <div className="w-full justify-start text-dark text-sm font-medium truncate">
            {user.name}
          </div>
          <div className="w-full justify-center text-primary-text text-xs font-normal truncate">
            {user.email}
          </div>
        </div>
      </div>
